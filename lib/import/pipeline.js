...
${preProcessed.slice(0, 12000)}`;
  ...
  try {
    ...
    if (!items) { try { items = JSON.parse(response.content.trim()); } catch {} }
    if (!items) { const cb = response.content.match(/```(?:json)?\s*([\s\S]*?)```/); if (cb) { try { items = JSON.parse(cb[1].trim()); } catch {} } }
    ...
    console.warn('[pipeline] No valid JSON in conversation extraction. First 300 chars:', response.content.slice(0, 300));
  } catch (err) {
...
${rawContent.slice(0, 12000)}`;
  ...
  try {
    ...
    try {
      ...
      console.warn('[pipeline] Anthropic failed, trying OpenAI:', anthropicErr?.message?.slice(0, 100));
      const openaiAdapter = getAdapter('openai');
      response = await openaiAdapter.complete({
        messages: [{ role: 'user', content: profilePrompt }],
        model: 'gpt-4o-mini',
        maxTokens: 2048,
        temperature: 0.2,
    ...
    console.log(`[pipeline] Direct extraction response: ${response.content?.length || 0} chars, provider: ${response.provider}`);
    ...
    console.warn('[pipeline] No valid JSON found. Response length:', response.content?.length || 0, 'First 500 chars:', (response.content || '').slice(0, 500));
  } catch (err) {
    console.error('[pipeline] Direct extraction failed:', err?.message, err?.stack?.slice(0, 200));
  }
