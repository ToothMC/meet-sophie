...
export class SophieTabManager {
  ...
  async #tryBecomePrimary() {
    ...
    // Try non-blocking: ifAvailable = true
    // Retry up to 3 times with short delays to handle same-tab navigation
    // where the previous page's lock needs a moment to be released by the browser.
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      // Brief wait before retry — gives browser time to release stale locks
      if (attempt < 2) {
        await new Promise(r => setTimeout(r, 300));
      }
    }

    // All retries failed — genuinely another tab
    this.#onDuplicateTab();
  }
