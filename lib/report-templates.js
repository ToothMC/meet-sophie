...
export function renderScorecardReport(data, lang = 'de') {
  ...
  }[lang] || L?.de || { situation:'Situation', goal:'Ziel', mainProblem:'Hauptproblem', overall:'Overall Score', confidence:'Confidence', verdict:'Verdict', strengths:'Stärken', weaknesses:'Schwächen', nextSteps:'Nächste Schritte', footer:'Erstellt mit Sophie · meet-sophie.com' };
  const contextLines = [
...
${listBlock('Stärken', data.strengths, SC.strength)}
${listBlock('Schwächen', data.weaknesses, SC.weakness)}
${listBlock('Nächste Schritte', data.nextSteps, SC.action)}

  <div style="${SC.footer}">Erstellt mit Sophie · meet-sophie.com</div>
</div>`;
}
