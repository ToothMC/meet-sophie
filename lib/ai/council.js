...
import {
  ADVISORS, ECO_ADVISORS, REVIEWER_CHAIN, ECO_REVIEWER_CHAIN,
  LIMITS, TIMEOUTS, RATE_LIMITS, BRIEFING_LIMITS,
  MIN_DISTINCT_PROVIDERS, MAX_COUNCIL_DEPTH,
  getCouncilConfig, shouldRunCouncil,
...
export function validateBriefing(obj) {
  ...
  for (const k of Object.keys(obj)) if (!BRIEFING_KEYS.includes(k)) return { ok: false, reason: `unknown_field:${k}` };
...
function logRun(supabase, { userId, sessionId, mode, channel, result, costUsd, latencyMs }) {
  if (!supabase) return;
  const meta = {
...
export async function runCouncil(opts) {
  ...
  const supabase = opts.supabase || (process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY ? getServiceClient() : null);
  const costs = [];
  const finish = (result) => {
    ...
    logRun(supabase, { userId, sessionId, mode, channel, result, costUsd, latencyMs });
  ...
  const advisorTimeout = mode === 'deep' ? TIMEOUTS.deepAdvisorMs : TIMEOUTS.advisorMs;
  const reviewerTimeout = mode === 'deep' ? TIMEOUTS.deepReviewerMs : TIMEOUTS.reviewerMs;
  ...
  const settled = await Promise.allSettled(selection.advisors.map(async ({ provider, model }) => {
    ...
    const resp = await withTimeout(adapter.complete({ messages: advisorMessages, model, maxTokens: 1024, temperature: 0.5 }), advisorTimeout);
  ...
  const advisorsMeta = selection.advisors.map((a, i) => ({
    ...
    ok: settled[i].status === 'fulfilled' && !!settled[i].value.text,
    latencyMs: settled[i].status === 'fulfilled' ? settled[i].value.latencyMs : 0,
  }));
  ...
  const chain = orderReviewerChain(isEco ? ECO_REVIEWER_CHAIN : REVIEWER_CHAIN, healthMap, excludeProvider);
  const callChain = async (messages, role, temperature) => {
    for (const { provider, model } of chain) {
      const t0 = Date.now();
      try {
        const resp = await withTimeout(getAdapter(provider).complete({ messages, model, maxTokens: 1500, temperature }), reviewerTimeout);
  ...
  let review = null;
  if (mode === 'deep') {
    const raw = await callChain(buildReviewPrompt({ question: q.text, answers }), 'review', 0.3);
  ...
  const synthesisRaw = await callChain(buildSynthesisPrompt({ question: q.text, answers, review }), 'synthesis', 0.3);
