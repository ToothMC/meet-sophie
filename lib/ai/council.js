...
import {
  ...
  getCouncilConfig, shouldRunCouncil, pickReviewerChain, timeBudget, remainingBudget,
...
// Models express the same meaning in different shapes: a string where a list was
// asked for, strings where objects were asked for, localised enum values. These
// helpers normalise shape — not content. Every value is coerced to a known type,
// clamped to length, and the briefing is rebuilt from the whitelist only.
function toText(v, max) {
  ...
  if (Array.isArray(v)) return v.map(x => toText(x, max)).filter(Boolean).join(' ').trim().slice(0, max);
  if (v && typeof v === 'object') {
    for (const k of ['text', 'value', 'summary', 'content', 'recommendation', 'position', 'point']) {
      if (typeof v[k] === 'string') return v[k].trim().slice(0, max);
    }
  }
  return '';
}

function toList(v, maxItems, maxLen) {
  const raw = Array.isArray(v) ? v : (v == null || v === '' ? [] : [v]);
  return raw.slice(0, maxItems).map(x => toText(x, maxLen)).filter(Boolean);
}

const ENUM_ALIASES = {
  high: 'high', hoch: 'high', stark: 'high', strong: 'high',
  medium: 'medium', mittel: 'medium', moderate: 'medium', mixed: 'medium', partial: 'medium',
  low: 'low', niedrig: 'low', gering: 'low', schwach: 'low', weak: 'low', none: 'low',
};

function toEnum3(v) {
  const key = String(v ?? '').trim().toLowerCase();
  return ENUM_ALIASES[key] || (ENUM3.includes(key) ? key : null);
}

function toDissent(v, maxItems, maxLen) {
  const raw = Array.isArray(v) ? v : (v == null || v === '' ? [] : [v]);
  const out = [];
  for (const d of raw.slice(0, maxItems)) {
    if (typeof d === 'string') {
      const position = toText(d, maxLen);
      if (position) out.push({ position, reason: '', severity: 'medium' });
      continue;
    }
    if (d && typeof d === 'object') {
      const position = toText(d.position ?? d.point ?? d.text ?? d.claim, maxLen);
      if (!position) continue;
      out.push({
        position,
        reason: toText(d.reason ?? d.rationale ?? d.why, maxLen),
        severity: toEnum3(d.severity) || 'medium',
      });
    }
  }
  return out;
...
// Fail closed on substance, not on shape: without a recommendation there is
// nothing to brief Sophie with and the run degrades. Everything else is
// normalised or defaulted. Unknown fields never survive — the returned object is
// rebuilt from known keys only.
export function validateBriefing(obj) {
  if (!obj || typeof obj !== 'object' || Array.isArray(obj)) return { ok: false, reason: 'not_an_object' };

  const recommendation = toText(obj.recommendation ?? obj.empfehlung, BRIEFING_LIMITS.recommendation);
  if (!recommendation) return { ok: false, reason: 'missing_recommendation' };

  const spokenSummary = toText(obj.spokenSummary ?? obj.summary, BRIEFING_LIMITS.spokenSummary)
    || recommendation.slice(0, BRIEFING_LIMITS.spokenSummary);

  return {
    ok: true,
    briefing: {
      recommendation,
      consensus: toList(obj.consensus, BRIEFING_LIMITS.listItems, BRIEFING_LIMITS.listItem),
      dissent: toDissent(obj.dissent, BRIEFING_LIMITS.listItems, BRIEFING_LIMITS.listItem),
      uncertainty: toList(obj.uncertainty, BRIEFING_LIMITS.listItems, BRIEFING_LIMITS.listItem),
      criticalAssumptions: toList(obj.criticalAssumptions, BRIEFING_LIMITS.listItems, BRIEFING_LIMITS.listItem),
      agreement: toEnum3(obj.agreement) || 'medium',
      assessmentConfidence: toEnum3(obj.assessmentConfidence) || 'medium',
      spokenSummary,
    },
  };
}
...
export async function runCouncil(opts) {
  ...
  const budget = timeBudget({ mode, channel });
  ...
  const settled = await Promise.allSettled(selection.advisors.map(async ({ provider, model }) => {
    ...
    const resp = await withTimeout(
      adapter.complete({ messages: advisorMessages, model, maxTokens: ADVISOR_MAX_TOKENS, temperature: 0.5 }),
      remainingBudget(started, budget.totalMs, budget.advisorMs),
    );
  ...
  const callChain = async (messages, role, temperature, timeoutMs, json = false) => {
    for (const { provider, model } of chain) {
      // Without this the chain multiplies: each provider got the full
      // per-attempt budget, so three retries could outlive the function itself.
      const left = remainingBudget(started, budget.totalMs, timeoutMs);
      if (left < MIN_ATTEMPT_MS) {
        console.warn(`[council] ${role}: out of time before ${provider}/${model} (${left}ms left)`);
        break;
      }
      const t0 = Date.now();
      try {
        const resp = await withTimeout(getAdapter(provider).complete({ messages, model, maxTokens: REVIEWER_MAX_TOKENS, temperature, json }), left);
  ...
  if (!parsed.ok) {
    console.warn(`[council] briefing rejected (${parsed.reason}); raw:`, String(synthesisRaw).slice(0, 400));
  ...
  console.log(`[council] ok — ${advisorsMeta.filter(a => a.ok).map(a => a.provider).join(',')} | agreement=${briefing.agreement} | spoken="${briefing.spokenSummary}" | rec="${briefing.recommendation.slice(0, 300)}"`);
