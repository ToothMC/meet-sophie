...
import {
  ...
  LIMITS, RATE_LIMITS, BRIEFING_LIMITS,
...
export async function runCouncil(opts) {
  ...
  const settled = await Promise.allSettled(selection.advisors.map(async ({ provider, model }) => {
    ...
    const resp = await withTimeout(adapter.complete({ messages: advisorMessages, model, maxTokens: 1024, temperature: 0.5 }), budget.advisorMs);
  ...
  const callChain = async (messages, role, temperature, timeoutMs) => {
    for (const { provider, model } of chain) {
      const t0 = Date.now();
      try {
        const resp = await withTimeout(getAdapter(provider).complete({ messages, model, maxTokens: 1500, temperature }), timeoutMs);
  ...
  const synthesisRaw = await callChain(buildSynthesisPrompt({ question: q.text, answers, review }), 'synthesis', 0.3, budget.synthesisMs);
