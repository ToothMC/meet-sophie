...
export class BudgetAdapter {
  ...
  async _healthGoogle() {
    const start = Date.now();
    try {
      // Use models.list endpoint — free, no tokens, always works if API key is valid
      const url = `https://generativelanguage.googleapis.com/v1beta/models?key=${process.env.GOOGLE_AI_API_KEY}&pageSize=1`;
      const resp = await fetch(url);
