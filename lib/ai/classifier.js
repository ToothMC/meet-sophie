...
export function route(ctx) {
  ...
  // Eco Mode → always cheapest models (same features, lower quality)
  if (ctx.ecoMode) {
    return {
  ...
  // Dimension 5: Premium tier → Claude Sonnet for best quality
  ...
  // Dimension 6: Abo tier → gpt-4o for quality on par with ChatGPT
  if (ctx.userTier === 'abo') {
    return {
  ...
  // Dimension 7: Long context → 1M context models
...
// Routing-only heuristic: decides WHICH model answers (quality tier), never
// whether other AIs get consulted — that is Sophie's own decision.
