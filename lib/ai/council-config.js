...
// Synthesis has to produce a full JSON briefing from every advisor answer —
// it needs markedly more headroom than a single advisor reply.
...
export function timeBudget({ mode = 'quick' } = {}) {
  return mode === 'deep'
    ? { advisorMs: TIMEOUTS.deepAdvisorMs, reviewMs: TIMEOUTS.deepReviewMs, synthesisMs: TIMEOUTS.deepSynthesisMs }
    : { advisorMs: TIMEOUTS.advisorMs, reviewMs: TIMEOUTS.synthesisMs, synthesisMs: TIMEOUTS.synthesisMs };
}
