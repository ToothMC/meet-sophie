...
export const TIMEOUTS = {
  advisorMs: 12000,
  // Measured: synthesis takes 9-11s. At 10s it succeeded once and failed once in
  // two consecutive production runs — a limit sitting exactly on the measured
  // duration is a coin flip, not a budget.
  synthesisMs: 14000,
  deepAdvisorMs: 12000,
  deepReviewMs: 10000,
  ...
  // The run deadline depends on what else shares the request, not just the mode:
  //   voice  /api/ai/council   (45s): council is the only work        → 30 + overhead
  //   chat   /api/chat         (45s): 5 primary + council + 12 formulation → 26
  //   deep   /api/ai/challenge (60s): council + 12 verdict            → 40
  totalQuickVoiceMs: 30000,
  totalQuickMs: 26000,
  totalDeepMs: 40000,
};
...
export function timeBudget({ mode = 'quick', channel = 'chat' } = {}) {
  if (mode === 'deep') {
    return { advisorMs: TIMEOUTS.deepAdvisorMs, reviewMs: TIMEOUTS.deepReviewMs, synthesisMs: TIMEOUTS.deepSynthesisMs, totalMs: TIMEOUTS.totalDeepMs };
  }
  return {
    advisorMs: TIMEOUTS.advisorMs,
    reviewMs: TIMEOUTS.synthesisMs,
    synthesisMs: TIMEOUTS.synthesisMs,
    totalMs: channel === 'voice' ? TIMEOUTS.totalQuickVoiceMs : TIMEOUTS.totalQuickMs,
  };
}
