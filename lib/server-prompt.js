...
export async function buildServerSystemPrompt({ supabase, user, sessionMode, brainstormConfig, language, conversationPolicy }) {
  ...
  if (user) {
    try {
      const [profRes, relRes, subRes, sessRes, importRes, _ltmRes, _stmRes, _reportsRes, _recentMsgsRes, calIntRes] = await Promise.all([
        ...
        // Active calendar integration check (parallel, zero extra latency)
      ...
      // Calendar context injection (immer wenn Integration aktiv, keine Keyword-Detection)
      if (calIntRes?.data) {
        try {
          const { getCalendarEventsForUser } = await import("./calendar-fetch.js");
          const calResult = await getCalendarEventsForUser(user.id, {
            days: 7,
            language: profile.preferred_language || 'de',
          });
          if (calResult?.text) {
            importedContext = "\n\n" + calResult.text + importedContext;
          }
        } catch (e) {
          console.warn("[server-prompt] Calendar context error:", e?.message);
        }

        // Contacts context (wenn Scope contacts.readonly vorhanden)
        const hasContactsScope = (calIntRes.data.scopes || []).some(s => s.includes('contacts'));
        if (hasContactsScope) {
          try {
            const { getContactsForUser } = await import("./contacts-fetch.js");
            const contactsResult = await getContactsForUser(user.id, {
              language: profile.preferred_language || 'de',
            });
            if (contactsResult?.text) {
              importedContext = "\n\n" + contactsResult.text + importedContext;
            }
          } catch (e) {
            console.warn("[server-prompt] Contacts context error:", e?.message);
          }
        }
      }
    } catch (e) {
