...
export function buildSophiePrompt({
  ...
  const memCtx = _memoryContext(user, memory, tier, channel, structuredMemory, recentMemories, recentReports, recentConversations);
...
// Layer 4 — Memory Context (tier-scoped)
// ---------------------------------------------------------------------------
function _memoryContext(user, memory, tier, channel, structuredMemory = null, recentMemories = [], recentReports = [], recentConversations = []) {
...
- If the user mentions something that updates or contradicts this, trust what they say now.`;
  ...
  // Recent reports (from conversation_outputs)
  let reportsBlock = null;
  if (recentReports?.length > 0 && tier !== "free") {
    const entries = recentReports.map(r => {
      const date = r.date ? new Date(r.date).toISOString().slice(0, 10) : "?";
      const mode = r.mode ? ` (${r.mode})` : "";
      // Strip HTML from report summary
      const reportLimit = channel === "voice" ? 150 : 300;
      const cleanSummary = r.summary.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, reportLimit);
      return `[${date}${mode}] ${r.title}: ${cleanSummary}`;
    });
    reportsBlock = `PAST REPORTS (Sophie's analysis from previous sessions):\n${entries.join("\n")}\n\nRules:\n- You wrote these reports. You can reference insights from them when relevant.\n- If the user asks about a past session, use report context to answer.`;
  }

  // Recent conversation snippets (from conversation_messages — chat + voice merged)
  let conversationsBlock = null;
  if (recentConversations?.length > 0 && tier !== "free") {
    // Group by session, show last 3 sessions max
    const bySession = {};
    for (const m of recentConversations) {
      const sid = m.session_id || "unknown";
      if (!bySession[sid]) bySession[sid] = [];
      bySession[sid].push(m.text);
    }
    const maxSessions = channel === "voice" ? 2 : 3;
    const maxMsgsPerSession = channel === "voice" ? 3 : 5;
    const msgSliceLen = channel === "voice" ? 80 : 120;
    const sessionIds = Object.keys(bySession).slice(0, maxSessions);
    const entries = sessionIds.map((sid, i) => {
      const msgs = bySession[sid].reverse(); // chronological
      const preview = msgs.slice(0, maxMsgsPerSession).map(t => `  - "${String(t).slice(0, msgSliceLen)}"`).join("\n");
      return `Session ${i + 1}:\n${preview}`;
    });
    if (entries.length > 0) {
      conversationsBlock = `WHAT THE USER SAID RECENTLY (their own words from past sessions):\n${entries.join("\n")}\n\nRules:\n- Use these to understand context and pick up threads naturally.\n- Never quote back verbatim. Never say "you said X last time" unless they ask.`;
    }
  }

  return [addressingBlock, profileBlock, structuredBlock, reportsBlock, conversationsBlock, memoryFileBlock, recentMemoriesBlock, memoryBlock].filter(Boolean).join("\n\n");
}
