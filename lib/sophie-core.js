...
- If the user asks about something from a past session, THEN you may reference it subtly.`;
  ...
  const memoryFileBlock = rawMemoryFile
    ? (() => {
        // Truncate to max 3000 chars to stay token-efficient
        const truncated = rawMemoryFile.length > 3000
          ? rawMemoryFile.slice(-3000)
          : rawMemoryFile;
...
- If the user mentions something that updates or contradicts this, trust what they say now.`;
  ...
  return [addressingBlock, profileBlock, memoryFileBlock, memoryBlock].filter(Boolean).join("\n\n");
}
...
Do NOT produce any opening message from these system instructions.
The frontend will trigger your first turn explicitly via response.create with mode-specific instructions.
Until then, remain silent. After the opening turn, resume normal conversation behavior.`;
  }
...
Do NOT produce any opening message from these system instructions.
The frontend will trigger your first turn explicitly via response.create with brainstorm-specific instructions.
Until then, remain silent. After the opening turn, follow the BRAINSTORM MODE block for subsequent turns.`;
  }
