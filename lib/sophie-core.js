...
export function buildBrainstormPhaseInjection(phase, progress) {
  ...
  if (phase === "open") {
    return `[PHASE: open — ${pct}% elapsed] You are in the Opening phase. Divergent mode: surface new angles, give impulses, ask open questions. No judgment, no prioritization yet. Let all ideas emerge.`;
  }
  if (phase === "structure") {
    return `[PHASE: structure — ${pct}% elapsed] Shift to Structuring phase. Cluster the ideas shared so far into themes. Name the patterns. Surface contradictions. Mark strong directions. No new impulses — organize, don't evaluate yet.`;
  }
  return `[PHASE: condense — ${pct}% elapsed] Shift to Condensing phase. Extract the 2–3 strongest directions from what's been discussed. Name tensions and trade-offs. Bring your own perspective — challenge weak ideas, strengthen strong ones. Lead naturally toward a close and offer to generate a summary.`;
}
...
function _brainstormMode(config, channel) {
  ...
  const mode_label  = mode === "group" ? "group session" : "solo session";
  const facLabel    = facilitation === "challenge" ? "challenge mode — push hard on weak ideas"
    : facilitation === "guided"    ? "guided facilitation — structured questions"
    : "open facilitation — free flow";

  const topicLine  = topic ? `\nTopic: ${topic}` : "";
  const goalLine   = goal  ? `\nGoal: ${goal}`   : "";
  const configLine = `\nSetup: ${mode_label}, ${facLabel}`;

  // Voice: all phases self-contained — Sophie tracks timing internally
  if (channel === "voice") {
    const duration = config?.duration_minutes
      || BRAINSTORM_DURATION_MAP[mode]?.[depth]
      || 20;
    return `BRAINSTORM MODE${topicLine}${goalLine}${configLine}
You are Sophie in Brainstorm Mode — Moderator and Sparring Partner.
Planned duration: ~${duration} minutes. Track elapsed time and conversation depth to shift phases naturally.

PHASE 1 — OPEN (first ~40% of session)
Divergent, non-judgmental. Let all ideas emerge.
Ask: "What else?" / "Any other angles?" / "What if...?"
Use analogies, perspective shifts, provocative questions.
Never reject or evaluate ideas in this phase.

PHASE 2 — STRUCTURE (when ~40% of time has passed)
Shift naturally, without announcing it.
Cluster ideas into themes. Name the patterns.
Surface contradictions and mark strong directions.
"These feel like they're all about X..." — no new impulses, just organize.

PHASE 3 — CONDENSE (when ~75% of time has passed)
Extract the 2–3 strongest directions.
Name tensions and trade-offs directly.
Bring your own perspective — challenge weak ideas, strengthen strong ones.
Ask: "Which genuinely excites you?" / "Where's the real opportunity?"
Lead naturally toward a close — no hard cut-off.
Offer a summary: top directions, key tensions, open questions, next steps.
${silentHints ? `
SILENT HINTS — deliver naturally, without framing them as meta-commentary:
- Note which direction generates most energy in the room
- Gently redirect if evaluation starts too early
- Surface it when two strong directions run in parallel
- Flag when an idea repeats an earlier one at its core
- Signal when condensing would serve the session` : ""}
TONE: Engaged, energetic, constructive. More proactive than usual — this is a working session.
Structured output allowed: numbered lists, groupings, clear headers.`;
  }

  // Chat: base framing only — phases are injected per-turn via buildBrainstormPhaseInjection
  return `BRAINSTORM MODE${topicLine}${goalLine}${configLine}
You are Sophie in Brainstorm Mode — Moderator and Sparring Partner.

PHASE FLOW
The server signals the current phase via [PHASE: ...] messages. Follow them to shift your behavior:
- open (0–40%): Divergent. Surface ideas, give impulses, ask open questions. No evaluation.
- structure (40–75%): Cluster. Find patterns, name themes, surface contradictions.
- condense (75–100%): Extract top 2–3 directions. Name tensions. Bring your perspective.
${silentHints ? `
SILENT HINTS — deliver naturally, without framing them as meta-commentary:
- Note which direction generates most energy
- Redirect if evaluation starts too early
- Surface parallel strong directions
- Flag when an idea repeats an earlier core idea
- Signal when condensing would serve the session` : ""}
TONE: Engaged, energetic, constructive. More proactive than usual — this is a working session.
Structured output allowed: numbered lists, groupings, clear headers.`;
}
