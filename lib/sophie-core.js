...
- If the user mentions something that updates or contradicts this, trust what they say now.`;
  ...
  if (recentReports?.length > 0 && tier !== "free") {
    ...
    const parts = [];
    if (tierNEntries.length > 0) {
      parts.push(`RECENT CONTEXT (last ${tierNEntries.length} ${currentMode} session${tierNEntries.length === 1 ? "" : "s"} — detailed recap):\n\n${tierNEntries.join("\n\n")}`);
    }
    if (tierKEntries.length > 0) {
      parts.push(`EARLIER SESSIONS (chronology only — don't reference unless asked):\n${tierKEntries.join("\n")}`);
    }

    if (parts.length > 0) {
      tierBlock = `${parts.join("\n\n")}\n\nRules:\n- RECENT CONTEXT holds structured recaps you wrote at session end. Use to pick up threads naturally.\n- EARLIER SESSIONS is timeline reference only — never bring them up unless the user asks.\n- Never quote or recite these verbatim. Never say "you said X last time" unless they ask.`;
    }
  }
