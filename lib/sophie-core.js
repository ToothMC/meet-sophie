...
- If the user mentions something that updates or contradicts this, trust what they say now.`;
  ...
  if (recentReports?.length > 0 && tier !== "free") {
    ...
    const isSameMode = (rowMode) => {
      const m = (rowMode || "talk").toLowerCase();
      // Treat null/"voice" rows as talk for backward compat
      const normalized = (m === "voice" || m === "") ? "talk" : m;
    ...
    const fmtMode = (m) => {
      const norm = (m || "talk").toLowerCase();
      return (norm === "voice" || norm === "") ? "talk" : norm;
    };
