...
- If the user asks about something from a past session, THEN you may reference it subtly.`;
  ...
  const memoryFileBlock = rawMemoryFile
    ? (() => {
        // Truncate to max 4000 chars — structured dossier should stay well under this
        // slice(0, ...) keeps the top (Identity, Family) which is most important
        const truncated = rawMemoryFile.length > 4000
          ? rawMemoryFile.slice(0, 4000)
          : rawMemoryFile;
...
- If the user mentions something that updates or contradicts this, trust what they say now.`;
  ...
  if (recentReports?.length > 0 && tier !== "free") {
    const entries = recentReports.map(r => {
      ...
      const cleanSummary = r.summary.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, 300);
