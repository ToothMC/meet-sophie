...
export function buildSophiePrompt({
  ...
  const memCtx = _memoryContext(user, memory, tier, channel, structuredMemory, recentMemories, recentReports);
...
// Layer 4 — Memory Context (tier-scoped)
// ---------------------------------------------------------------------------
function _memoryContext(user, memory, tier, channel, structuredMemory = null, recentMemories = [], recentReports = []) {
...
- If the user mentions something that updates or contradicts this, trust what they say now.`;
  ...
  return [addressingBlock, profileBlock, structuredBlock, reportsBlock, memoryFileBlock, recentMemoriesBlock, memoryBlock].filter(Boolean).join("\n\n");
}
