...
function openReportOverlay(reportData, options = {}) {
    ...
    <div class="sro-container">
      <div class="sro-header">
        ...
        <div class="sro-title-row">
          ...
          <button class="sro-close" aria-label="Schließen">&times;</button>
        </div>
      </div>
      <div class="sro-content"></div>
      <div class="sro-export-bar"></div>
      ${onDelete ? '<div class="sro-delete-row"><button class="sro-delete-btn">' + (lang === "de" ? "Löschen" : "Delete") + '</button></div>' : ''}
    </div>
  `;
  ...
  if (reportData.html) {
    ...
    contentEl.innerHTML = `<p style="color:#999;text-align:center;padding:40px 0;">${lang === "de" ? "Kein Report vorhanden." : "No report available."}</p>`;
  }
  ...
  if (onDelete) {
    ...
    if (deleteBtn) {
      deleteBtn.addEventListener("click", async () => {
        const confirmMsg = lang === "de" ? "Report wirklich löschen?" : "Really delete this report?";
