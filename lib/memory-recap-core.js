...
function buildSystemPrompt(lang) {
  ...
  return [
    ...
    "- Frame points as observations, not decisions. Use verbs like 'mentioned', 'discussed', 'asked about', 'explored'. Avoid 'wants', 'decided', 'plans to' UNLESS the user said this explicitly and verbatim.",
    "- Never attribute strong intent to the user from something Sophie said in response. Only user-stated positions count.",
    "",
    "TEMPLATE (fill in between angle brackets, keep labels):",
    "Thema: <one short phrase naming the topic>",
    "User-Position: <what the user expressed or asked about, one line — NOT what Sophie proposed>",
    "Kernpunkte:",
    "  • <observation 1>",
    "  • <observation 2>",
    "  • <observation 3 if relevant>",
    "Offen: <open thread the user did not resolve, or ->",
...
function detectLanguage({ messages, structuredSummary }) {
  ...
  // Use ALL user turns — short test sessions may have only 1-2 turns. Short replies
  // ("Ja", "ok") rarely match markers; skip those to weight content turns more.
  const userText = messages
    ...
    .join(" ")
    .toLowerCase();
  // German: pronouns, modal verbs, common conjunctions — incl. plural inflections
  if (/\b(und|aber|ich|du|wir|ihr|nicht|habe[nt]?|hast|hat|war|werde[nt]?|wirst|wird|kann|können|möchte|sind|ist|bin|mit|auch|dass|über|noch|dich|dir|mich|mir|sehr|gerade|schon|jetzt)\b/i.test(userText)) return "de";
  // French: pronouns, common verbs, contractions
  if (/\b(et|je|tu|nous|vous|pas|avons?|avez|pouvez|serait|dans|avec|mais|pour|qui|que|quoi|c'est|j'ai|n'est)\b/i.test(userText)) return "fr";
  return "en";
}
...
export async function runRecapForSession({ supabase, sessionId, userId, force = false }) {
  ...
  if (outputRow?.id) {
    // .select() forces the response to include affected rows so we can
    // detect the silent no-op that happens when RLS denies the write.
    ...
    if (!updated || updated.length === 0) {
      return { ok: false, error: "persist: update affected 0 rows (likely RLS policy missing)" };
    }
  } else {
    ...
    if (!inserted || inserted.length === 0) {
      return { ok: false, error: "persist: insert affected 0 rows (likely RLS policy missing)" };
    }
  }
