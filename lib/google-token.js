  ...
  try {
    ...
    if (isValid) {
      try {
        ...
        // Decrypt failures are permanent (wrong key / corrupted cipher) —
        // deactivate so the integration stops polluting logs on every call.
        await writeError(supabase, userId, provider, `Decrypt access_token failed: ${e?.message}`, { deactivate: true });
        return null;
      }
    }
    ...
    try {
      ...
      // Same logic as access_token: permanent failure, deactivate.
      await writeError(supabase, userId, provider, `Decrypt refresh_token failed: ${e?.message}`, { deactivate: true });
      return null;
    }
...
async function writeError(supabase, userId, provider, message, opts = {}) {
  try {
    const update = { last_error: message };
    if (opts.deactivate) update.is_active = false;
    await supabase.from('user_integrations')
      .update(update)
      .eq('user_id', userId).eq('provider', provider);
  } catch (_) {
