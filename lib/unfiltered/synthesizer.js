...
export async function synthesizeBriefing(rawSignals, { language = "de", max_stories = 5 } = {}) {
  ...
  // Index der erlaubten Source-URLs — wir verwerfen nachher jede Story,
  // deren sources nicht in diesem Set sind. So fängt der Code Halluzinationen
  // auch dann ab, wenn das LLM den System-Prompt ignoriert.
  ...
  try {
    ...
    // den ursprünglichen Signalen vorkam. Hilft auch wenn der LLM den
    // System-Prompt ignoriert.
    const grounded = stories.filter(s => {
      ...
      // Auf nur gegroundete sources reduzieren
