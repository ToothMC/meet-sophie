...
export async function getEmail(token, messageId) {
  try {
    // format=full aber nur Text-Parts holen (keine Attachment-Binaries)
    // Das verhindert OOM bei Emails mit grossen Anhaengen
    const params = new URLSearchParams({
      format: 'full',
      fields: 'id,threadId,snippet,payload(headers,mimeType,body,parts(mimeType,body,parts(mimeType,body)))',
    });
