-- Remove legacy 50-free-token default from the two billing RPCs.
--
-- Context: SG-2 from the audit. The JS code paths were fixed in
-- commit 219044d to use DEFAULT_FREE_TOKENS (= 0), but the two DB
-- functions still had the old hardcoded 50:
--   - deduct_tokens: COALESCE(r.free_tokens_total, 50)
--   - meeting_create_with_token_gate: same COALESCE twice, plus a
--     `VALUES (p_user_id, 50, 0, 0, 0, 0)` when auto-creating a row
--     for a brand-new user via meeting entry.
--
-- Result before this migration: users who hit the meeting flow first
-- could still be seeded with 50 free tokens server-side, bypassing
-- the JS-side fix. This migration closes the gap so that the DB and
-- JS agree on "zero free tokens by default".
--
-- Everything else in the function bodies is byte-identical to
-- 20260420_production_rpcs_recovered.sql — only the four numeric
-- literals changed 50 → 0.

BEGIN;

-- ─────────────────────────────────────────────
-- deduct_tokens — fix COALESCE(..., 50) → COALESCE(..., 0)
-- ─────────────────────────────────────────────
...
BEGIN
  ...
  free_rem := GREATEST(0, COALESCE(r.free_tokens_total, 0) - COALESCE(r.free_tokens_used, 0));
...
-- ─────────────────────────────────────────────
-- meeting_create_with_token_gate — three 50 → 0 changes
--   1. COALESCE(u.free_tokens_total, 50) in idempotent-hit branch
--   2. INSERT INTO user_usage ... VALUES (..., 50, 0, 0, 0, 0) for new users
--   3. COALESCE(r.free_tokens_total, 50) in main balance read
-- ─────────────────────────────────────────────
...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL AND p_idempotency_key != '' THEN
    ...
    IF FOUND THEN
      ...
      SELECT GREATEST(0, COALESCE(u.free_tokens_total, 0) - COALESCE(u.free_tokens_used, 0))
  ...
  IF NOT FOUND THEN
    -- New user: create usage row with defaults (zero free tokens since
    -- the free plan was removed; see DEFAULT_FREE_TOKENS in lib/billing-constants.js)
