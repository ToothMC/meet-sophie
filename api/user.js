...
async function handleUsage(req, res) {
  try {
    ...
    const sec = Math.max(0, Math.min(60 * 60, Number(seconds_used || 0)));
    ...
    if (!usage) {
      const ins = await supabase.from("user_usage").insert({
        user_id: user.id,
        free_seconds_total: 120, free_seconds_used: 0,
        paid_seconds_total: 0, paid_seconds_used: 0, topup_seconds_balance: 0,
      }).select().maybeSingle();
    ...
    const upd = await supabase
      .from("user_usage")
      .update({ free_seconds_used: newFreeUsed, paid_seconds_used: newPaidUsed, topup_seconds_balance: newTopupBal })
      .eq("user_id", user.id)
      .select("free_seconds_total, free_seconds_used, paid_seconds_total, paid_seconds_used, topup_seconds_balance")
      .maybeSingle();
